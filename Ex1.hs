...
numEvens numbers
    | null numbers = 0
    | (even (head numbers)) = 1 + (numEvens (tail numbers))
    | otherwise = numEvens (tail numbers)
    -- if null numbers
    -- then
    --   ...
    -- else
...
numManyEvens listsOfNums
    | null listsOfNums = 0
    | (numEvens (head listsOfNums)) >= 3 = 1 + (numManyEvens (tail listsOfNums))
    | otherwise = numManyEvens (tail listsOfNums)
