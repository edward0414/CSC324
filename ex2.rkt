#lang racket #| â˜… CSC324 Fall 2019: Exercise 2 â˜… |#
#|
Module: ex2
...
;-------------------------------------------------------------------------------
; â˜… Task 2: Detecting tail calls â˜…
;-------------------------------------------------------------------------------
...
#|(define (tail-calls expr)
  (cond
    [(null? expr) null]
    [(integer? expr) (list expr)]
    [(list? expr) (list (tail-calls (rest expr)))]
    [else expr]))|#
...
(define (tail-calls expr)
  (cond
    ...
    [(list? expr)
     (cond
       [(equal? (first expr) 'if) (append (tail-calls (third expr)) (tail-calls (fourth expr)))]
       [else (list expr)]
       )]
    [else expr]))
