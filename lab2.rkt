  ...
  (cond
    ...
    [else
     (let ([first-element (first lst)])
       (cond
         [(pred first-element) (+ 1 (num-pred pred (rest lst)))]
         [ else (num-pred pred (rest lst))]
         )
       )
     ]))
...
(define num-many-evens-1 (make-counter valid-list))
...
(module+ test
  ...
  (test-equal? "num-pred/num-evens-1"
               (num-evens-1 even? (list 1 2 3 4 5))
               (num-pred even? (list 1 2 3 4 5)))

  ; TODO: replace the ellipsis with the appropriate predicate for this test to pass.
  (test-equal? "num-pred/num-many-evens-1"
               (num-many-evens-1 valid-list (list (list 1 2 3 4 5) (list 2 4 6 10)))
...
(define (extract-ints prog)
    ...
    [(list? prog) (append (extract-ints (first prog)) (extract-ints (rest prog)))]
    [else (list)]))
...
(define (replace-324 datum)
  (cond
  [(null? datum) (list)]
  [(integer? datum) (if (= datum 324) 9000 datum)]
  [(list? datum) (cons (replace-324(first datum)) (replace-324 (rest datum)))]
  [else datum]))
